...
def shift_pixel(x, sf, upper_left=True):
    ...
    if x.ndim == 2:
        interp = RegularGridInterpolator((yv, xv), x, bounds_error=False, fill_value=None)
        x_shifted = interp(coords).reshape(h, w)
    elif x.ndim == 3:
        x_shifted = np.empty_like(x)
        for c in range(x.shape[2]):
            interp = RegularGridInterpolator((yv, xv), x[:, :, c], bounds_error=False, fill_value=None)
            x_shifted[:, :, c] = interp(coords).reshape(h, w)
    else:
